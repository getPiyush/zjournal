...
type ArticleEditorProps = {
  ...
  defaultArticle: ArticleT;
  availableComponents: string[];
  categories: string[];
  savedArticle?: ArticleT | null;
};
...
export default function ArticleEditor({
  ...
  useEffect(() => {
    if (savedArticle && article.id === "") {
      setArticle(savedArticle);
    }
  }, [savedArticle]);

  const [article, setArticle] = useState<ArticleT>(articleIn);
  const [content, setContent] = useState<ComponentObject[]>(articleIn.content);
  const [editMode, setEditMode] = useState(false);
  const [selectedElement, setSelectedElement] = useState("h2");
  const [editComponent, setEditcomponent] = useState<ComponentObject | null>(null);
  ...
  const onEditUpdate = (editComponent: ComponentObject) => {
  ...
  const onDelete = (editComponent: ComponentObject) => {
    setEditMode(false);
  ...
  const onAddElementClick = () => {
    ...
    const updatedContent = [...content, component];
  ...
  const showHdeOffCanvas = (flag: boolean) => {
      ...
      <div className="row mt-2 mb-2">
        <div className="col editor-action-col-start">
          <div className="dropdown" style={{ width: "100% !important" }}>
            ...
            >
              {availableComponents?.map((componentCode, index) => {
                return (
                  <li
